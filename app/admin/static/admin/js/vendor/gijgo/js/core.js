...
gj.core = {
    ...
    setChildPosition: function (mainEl, childEl) {
        ...
        if (mainElWidth < OPTIMAL_WIDTH) {
            childEl.style.maxWidth = mainElWidth + 'px';
        } else {
            childEl.style.maxWidth = OPTIMAL_WIDTH + 'px';
        }
...
gj.picker.methods = {
    ...
    initialize: function (picker, data, methods) {
        ...
        // }
        // TODO MODIFICATION
        if (input.parentElement.attributes.role.textContent !== 'wrapper') {
            wrapper = document.createElement('div');
            wrapper.setAttribute('role', 'wrapper');
            input.parentNode.insertBefore(wrapper, input);
            wrapper.appendChild(input);
